...
import { createStudentIfNotExists } from "@/sanity/lib/students/createStudentIfNotExists";
import { createEnrollment } from "@/sanity/lib/students/createEnrollment";
...
export async function createStripeCheckout(courseId: string, userId: string) {
  try {
    ...
    const session = await stripe.checkout.sessions.create({
      ...
      mode: "payment",
      success_url: `${baseUrl}/course/${slug.current}`,
      cancel_url: `${baseUrl}/course/${slug.current}?canceled=true`,
      metadata: {
        courseId: course._id,
        userId: userId,
      },
